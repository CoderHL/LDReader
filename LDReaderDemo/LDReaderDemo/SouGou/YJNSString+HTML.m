...
@implementation NSString (YJNSStringHTMLAdditions)
...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(HTMLEscapeMap*)table 
	...
	// this block is common between YJNSString+HTML and YJNSString+XML but
	...
	if (!buffer) {
		...
		if (!data) {
			...
			_YJDevLog(@"couldn't alloc buffer");
			return nil;
			// COV_NF_END
		}
	...
	if (!buffer || !data2) {
		// COV_NF_START
		_YJDevLog(@"Unable to allocate buffer or data2");
		return nil;
		// COV_NF_END
	}
	...
	for (NSUInteger i = 0; i < length; ++i) {
		...
		if (val || (escapeUnicode && buffer[i] > 127)) {
			...
			if (val) {
				...
				_YJDevAssert(escapeUnicode && buffer[i] > 127, @"Illegal Character");
